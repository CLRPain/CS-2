...
class mp3player(Fl_Window):
    ...
    def __init__(self, width, height, label="mp3player"):
        ...
        out = Fl_Output(0, 30, self.w(), 30) 
        self.brow = Fl_Hold_Browser(0, 60, self.w(), self.h()-130)
        looping = False 
        vol.scrollvalue(100, 3, 0, 103) 
        vol.slider_size(0.05) 
        mb = Fl_Menu_Bar(0,0, 700 ,30)
        mb.add("File/Open",ord('o'), mp3player.addSongs) #menu item to add songs
        mb.add("File/Clear",FL_BackSpace, mp3player.clearSongs) #menu item to clear songs
        self.end()
        
        #callbacks for buttons
        backbut.callback(mp3player.move_cb, -1)
        playbut.callback(mp3player.play_cb)
        forbut.callback(mp3player.move_cb, 1)
        stopbut.callback(mp3player.stop_cb)
        rembut.callback(mp3player.rem_cb)
        loopbut.callback(mp3player.loop_cb)
        self.brow.callback(mp3player.brow_cb)
        vol.callback(mp3player.vol_cb)
        self.resizable(self)
        ...
        self.brow.tooltip("Select song to play")
    ...
    def addSongs(self): #adding songs from a directory
        ...
        for songs in files:
            if songs[-4:] in (".mp3", ".ogg", ".wav"): #filter out all non audio files
                ...
                self.brow.add(self, songs) #add media to the browser
    ...
    def play_cb(self): #play the selected song
        mp3player.musicPlayer.play_item_at_index(
            mp3player.brow.value()-1)
