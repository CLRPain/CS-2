...
class Mine_Sweeper(Fl_Window):
	...
	def reveal(self, wid): #When a button is clicked
		...
		self.num(coord[0], coord[1]) #Sends to a recursive function to reveal more buttons if it can
	...
	def flagged(self,wid): #Flag function
			...
			if wid.value() == 1: # If not already a flag
				return
		...
		for x in range(-1,2):
			for y in range(-1,2):
				...
				if 0<=(row+x)<10 and 0<=(col+y)<10:
					...
					if (row+x, col+y) not in self.revealed_list: #makes it so it does recurse infintely
